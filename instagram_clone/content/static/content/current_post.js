    ...
    commentForms.forEach(form => form.addEventListener('submit', e => {
        ...
        $.ajax({
            ...
            success: function(response) {
                commentInput.value = "";
                console.log(response)
                commentLink.innerHTML = `${response.no_of_comments} Comments`;
...
const getData = (postID) => {
    $.ajax({
        type: 'GET',
        url: `/content/detail/${postID}`,
        success: function(response) {
            const data = response['data'];
            console.log(data);
            deleteLink.href = `/content/delete/${data.id}/`;
            post.innerHTML += post_list(data.id, data.user_img, data.author_id, data.img, data.liked, data.likes, data.author, data.no_of_comments, data.content, data.created);
                likeUnlikePosts();
                commentPost();
                postCommentsLoad();
        },
